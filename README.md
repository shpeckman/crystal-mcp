...
- List, read, and discover results carry cache hints: `cacheScope`
  (`MCP::CACHE_SCOPE_PRIVATE` by default, or `MCP::CACHE_SCOPE_PUBLIC`) and
  `ttlMs` (`0` = no caching). Return a full result struct from a handler to set
  them, e.g. `MCP::ReadResourceResult.new(contents: [...],
  cache_scope: MCP::CACHE_SCOPE_PUBLIC, ttl_ms: 60_000)`.
...
```
src/crystal-mcp.cr          shard entry point (require "crystal-mcp" loads this)
src/mcp.cr                  library root (requires everything)
src/mcp/version.cr          library + protocol version constants, cache scope constants
...
gifts/server-template/      ready-to-fork server scaffold (shard.yml, src/, spec/)
gifts/crystal-mcp-builder.skill  packaged agent skill for building servers with this shard
```
